...
\hypersetup{
  pdfkeywords={},
  pdfsubject={},
  pdfcreator={Emacs 24.3.1 (Org mode 8.1)}}
\begin{document}
...
The \verb~geneRxCluster~ package provides some function for exploring
...
It might be helpful to look at these help pages brefly before getting
started:
...
of all the arguments used in the call to \verb~gRxCluster~ including two
that were filled in by default, and which will be discussed later on.
